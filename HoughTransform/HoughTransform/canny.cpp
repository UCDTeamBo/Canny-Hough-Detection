...
pixel_t *canny::load_bmp(const char *filename, bitmap_info_header_t *bitmapInfoHeader)
{
...
bool canny::save_bmp(const char *filename, const bitmap_info_header_t *bmp_ih, const pixel_t *data)
{
    ...
    const bmpfile_header_t bmp_fh = { offset + bmp_ih->bmp_bytesz, 0, 0, offset };
...
void canny::convolution(const pixel_t *in, pixel_t *out, const float *kernel,
 ...
 */
void canny::gaussian_filter(const pixel_t *in, pixel_t *out, const int nx, const int ny, const float sigma)
{
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++) {
            kernel[c] = exp(-0.5 * (pow(((i*1.0) - mean) / sigma, 2.0) + pow(((j*1.0) - mean) / sigma, 2.0))) / (2 * M_PI * sigma * sigma);
            c++;
        }
 ...
 */
pixel_t *canny::canny_edge_detection(const pixel_t *in, const bitmap_info_header_t *bmp_ih, const int tmin, const int tmax, const float sigma)
{
